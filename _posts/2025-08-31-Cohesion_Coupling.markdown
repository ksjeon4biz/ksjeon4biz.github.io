...
참고: 질문에 적으신 결함도는 보통 결합도(Coupling)를 의미하는 경우가 많습니다. 만약 
* 결함도(Defect Density)*를 뜻하셨다면 끝부분에 간단히 설명도 덧붙였습니다.
...
## 9. 개선 순서(리팩터링 로드맵)
   ...
   2) 시임(Seam) 만들기: 포트 인터페이스 도입 → 외부 의존 격리  
   3) 유스케이스 단위 테스트 작성 → 빠른 피드백 루프 확보  
   4) 어댑터 분리: Supabase, 결제, 외부 API 구현을 모듈로 격리  
   5) 플래그/거대 DTO 제거 → 전략/작은 값객체  
   6) RLS/RPC 강화 → 클라이언트 로직 단순화  
   7) 중복 제거 & 네이밍 정리 → 응집 강화  
