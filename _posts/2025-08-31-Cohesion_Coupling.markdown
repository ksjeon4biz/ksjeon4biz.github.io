...
참고: 질문에 적으신 결함도는 보통 결합도(Coupling)를 의미하는 경우가 많습니다. 만약 *결함도(Defect Density)*를 뜻하셨다면 끝부분에 간단히 설명도 덧붙였습니다.
