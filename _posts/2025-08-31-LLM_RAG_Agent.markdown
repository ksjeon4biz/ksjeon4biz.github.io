...
최신/사내 지식 접속 (모델 재학습 없이)

할루시네이션 감소 (출처로 근거 제시)

비용·속도 효율 (파라미터 업데이트 없이 확장)
...
문서 파이프라인: 수집→정규화→Chunking→임베딩→인덱스(벡터+BM25)

질문 처리: 재작성→하이브리드 검색→리랭크→컨텍스트 패킹

생성: 템플릿/포맷 가드, 출처 표기

보안: ACL 프리필터, PII 가드

관측성/평가: 리콜·Faithfulness·지연 대시보드

운영: 캐시·갱신 주기·비용 알람, 실패 시 폴백(“검색 결과 없음” 처리)
...
계획: (i) 정책 KB 검색, (ii) 지역별 배송비 SQL 조회, (iii) 최신 요율 필요 시 웹.
...
검증/합성: 상충 시 재조회 → 답변을 표로 정리 + 인용(문서·행·URL).

예시 B) “주문 취소하고 환불해줘”

RAG: 환불 정책 확인(수수료/마감시간).

SQL: 주문 상태·결제 수단 확인 → 승인(Authorize)만이면 VOID, 이미 Capture면 Refund.

결제 API 호출 → 재고 예약 해제(RPC) → 고객에게 결과 통지(이메일/푸시).

실패 시 보상 트랜잭션(SAGA) 실행.

예시 C) “상품 URL을 주면 카탈로그에 등록”

웹 스크래핑 → 스펙 추출 → 분류/속성 매핑(Strategy) → 이미지 업로드 → DB upsert → 번역 요청 큐잉.
